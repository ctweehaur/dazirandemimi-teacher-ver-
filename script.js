document.addEventListener("DOMContentLoaded", () => {
    ...
    if (typeof lessonTitle !== "undefined" && lessonTitle.length > 0 && titleContainer) {
        ...
        lessonTitle.forEach(word => {
            titleHtml += '<div class="word-card title-card" onclick="speakText(\'' + word[0] + '\')">' +
    ...
    if (typeof lessonData !== "undefined" && lessonData.length > 0 && contentContainer) {
        ...
        const renderSectionCard = (htmlContent, sectionNum) => {
            ...
            const appreciationText = (typeof lessonAppreciation !== "undefined" && lessonAppreciation[sectionNum - 1]) ? lessonAppreciation[sectionNum - 1] : "";
            
            let cardHtml = '<div class="section-card">' +
                           '<div class="section-tag">' + labelText + '</div>' +
                           '<div class="words-flow-container">' + htmlContent + '</div>';
            if (appreciationText) {
            ...
            contentContainer.appendChild(cardElement.firstElementChild);
        };
        ...
        for (let i = 0; i < lessonData.length; i++) {
            ...
            if (currentItem[0] === "\n") {
                ...
                cardHtmlBuffer += '<div class="word-card" onclick="speakText(\'' + currentItem[0] + '\')">' +
    ...
    if (quizContainer && typeof quizDataList !== "undefined" && quizDataList.length > 0) {
        ...
        quizDataList.forEach((quiz, index) => {
            ...
            quizHtml += '<div class="quiz-item" style="margin-bottom: 25px; padding-bottom: 20px; border-bottom: 1px dashed #eaefed; text-align: left;">' +
                        '<p class="quiz-title" style="font-size: 16px; font-weight: bold; color: #34495e; margin-bottom: 12px; line-height: 1.6;">' + (index + 1) + '. ' + formattedQuestion + '</p>' +
            ...
            quiz.options.forEach(option => {
                ...
                quizHtml += '<label style="display: flex; align-items: center; gap: 10px; padding: 10px 15px; background: #f8f9fa; border: 1px solid #e9ecef; border-radius: 8px; cursor: pointer; transition: all 0.2s;">' +
...
function submitAllAnswers() {
    ...
    quizDataList.forEach(quiz => {
        ...
        if (selectedRadio) {
            if (selectedRadio.value === quiz.answer) {
                score++;
